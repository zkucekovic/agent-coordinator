...
class TestEventLog(unittest.TestCase):
    ...
    def test_response_text_with_newlines_is_valid_json(self):
        log = EventLog(self._path)
        multiline = "line one\nline two\nline three"
        log.append(turn=1, agent="developer", task_id="t-001",
                   status_before="continue", status_after="review_required",
                   session_id="ses_b", response_text=multiline)
    ...
    def test_prompt_file_and_prompt_hash_present(self):
        log = EventLog(self._path)
        log.append(turn=2, agent="developer", task_id="t-002",
                   status_before="continue", status_after="review_required",
                   session_id="ses_c",
    ...
    def test_duration_seconds_present_and_rounded(self):
        ...
        log.append(turn=1, agent="architect", task_id="t-001",
                   status_before="continue", status_after="continue",
                   session_id="ses_d", duration_seconds=12.3456)
        events = log.read_all()
    ...
    def test_existing_fields_unchanged(self):
        log = EventLog(self._path)
        log.append(turn=5, agent="qa_engineer", task_id="t-003",
                   status_before="review_required", status_after="approved",
                   session_id="ses_g")
        events = log.read_all()
        r = events[0]
    ...
    def test_append_warning_creates_warning_record(self):
        log = EventLog(self._path)
        log.append_warning(turn=3, agent="developer", task_id="t-004",
                           error="Invalid transition: in_engineering → done")
        events = log.read_all()
    ...
    def test_append_warning_extra_fields(self):
        log = EventLog(self._path)
        log.append_warning(turn=2, agent="architect", task_id="t-005",
